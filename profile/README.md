# lhuasheng

We build _[one-line description of what your org does]_.

## Engineering at lhuasheng

We ship with a team of 6-15 engineers using **AI-augmented development**:
GitHub Copilot for code, Claude for code review, and a 5-gate SDLC that keeps
quality high without burning out our tech lead on manual review.

- **Gate 1: Intent** — every change starts from an approved issue spec.
- **Gate 2: Generation** — engineers own every line Copilot writes.
- **Gate 3: PR Review** — Claude pre-screens every PR via `/ai-review`.
- **Gate 4: CI** — lint, tests, security, PR-size, spec-link all block merge.
- **Gate 5: Deploy** — cut a tagged release, then ship it through your pipeline.

The full contributor guide lives at
[`.github/CONTRIBUTING.md`](https://github.com/lhuasheng/.github/blob/main/CONTRIBUTING.md).

## Our repos

| Repo | What it is |
|---|---|
| [`.github`](https://github.com/lhuasheng/.github) | Org-wide defaults, plus reasoning-heavy AI workflows (triage, digest, release notes) in `.github/workflows/` |
| [`shared-sdlc`](https://github.com/lhuasheng/shared-sdlc) | Reusable CI gates and AI-review actions |
| [`project-1`](https://github.com/lhuasheng/project-1) | _[describe the product]_ |

## Working with us

We're hiring engineers who are comfortable working alongside AI tools — not
fighting them, not blindly trusting them. If that sounds like you, see our
careers page at _[link]_.
